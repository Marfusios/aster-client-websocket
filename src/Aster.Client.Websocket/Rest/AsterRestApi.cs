...
{
    /// <summary>
    /// Aster base class for REST API.
    /// </summary>
    public abstract class AsterRestApi
    {
        private static readonly string UserAgent = "aster-client-websocket-dotnet/1.0";
        private readonly string? _apiKey;
        private readonly IAsterSignatureService _signatureService;
        private readonly string _baseUrl;
        private readonly HttpClient _httpClient;

        public AsterRestApi(HttpClient httpClient, string baseUrl, string? apiKey, string? apiSecret)
        {
            _httpClient = httpClient;
            _baseUrl = baseUrl;
            _apiKey = apiKey;
            _signatureService = new AsterHmac(apiSecret);
        }

        public AsterRestApi(HttpClient httpClient, string baseUrl, string? apiKey, IAsterSignatureService signatureService)
        {
            _httpClient = httpClient;
            _baseUrl = baseUrl;
            _apiKey = apiKey;
        ...
        private async Task<T> SendAsync<T>(string requestUri, HttpMethod httpMethod, object? content = null)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                {
                    var clientException = new AsterClientException($"Failed to map server response from '${requestUri}' to given type", -1, ex);
            ...
            AsterHttpException? httpException = null;
            ...
            if (400 <= statusCode && statusCode < 500)
            {
                ...
                {
                    httpException = new AsterClientException("Unsuccessful response with no content", -1);
                }
                else
                {
                    try
                    {
                        httpException = JsonConvert.DeserializeObject<AsterClientException>(contentString);
                    }
                    catch (JsonReaderException ex)
                    {
                        httpException = new AsterClientException(contentString, -1, ex);
                    }
                }
            }
            else
            {
                httpException = new AsterServerException(contentString);
            }
